<template>
  <div>
    <div class="header">
      <div class="text-md py-4" v-show="!props.pageProp">您当前查询的指标为: <span class="text-red-500 font-bold">{{ props.title
      }}</span></div>
      <div class="grid grid-cols-5 gap-4 w-full items-center">
        <a-range-picker :disabled="!props.showDatePick" v-model:value="form.pickDate" @change="datePickChange" />
        <a-select v-show="!props.pageProp" placeholder="请选择查询类别" v-model:value="form.typeName" :disabled="!props.showType"
        ...
        <!-- 专用于科室自助页面 -->
        <a-select v-show="props.pageProp" v-model:value="form.selfDeptName" placeholder="请输入科室名称" style="width: 100%"
