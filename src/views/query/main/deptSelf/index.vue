      ...
      <a-tab-pane v-for="item of tabs" :key="item.id" :tab="item.name">
        <div v-show="item.id === 1">
          <CommonPage :page-prop="props.pageProp" :show-date-pick="props.showDatePick" :show-period="props.showPeriod"
            ...
            <template #textTip>
              <span class="pl-2 text-green-500 text-md font-bold">(该查询根据所选择的科室返回门诊和住院数据,若要查询整个中心的数据,则不需要选择科室)
              </span>
              <span class="pl-2 text-black-500 font-bold text-md cursor-pointer hover:text-red-500"
                @click="openModal">全院汇总</span>
            </template>
          </CommonPage>
          <div class="modal">
            <a-modal v-model:visible="visible" title="全院数据汇总" @ok="handleOk">
              <p>Some contents...</p>
              <p>Some contents...</p>
              <p>Some contents...</p>
            </a-modal>
